...
const AnalysisReport = ({ data, standards, onReset }) => {
  ...
  return <div className="detailed-report-view">
    ...
    {data.playbackUrl && <section><h3>Slow Motion Review</h3><video ref={videoRef} src={data.playbackUrl.startsWith('/') ? `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'}${data.playbackUrl}` : data.playbackUrl} controls width="100%" onLoadedMetadata={() => changeRate(rate)} /><div><button onClick={() => frame(-1)}>◀ 1 frame</button>{[0.25, 0.5, 1].map((value) => <button key={value} onClick={() => changeRate(value)}>{value}×</button>)}<button onClick={() => frame(1)}>1 frame ▶</button></div></section>}
